...
def get_trending_coupons():
    if request.headers.get('Authorization'):
        ...
        list_coupon = db.engine.execute('SELECT *,\
                                        ...
                                        ((SELECT COUNT(*) FROM coupons_likes  WHERE coupons.coupon_id = coupons_likes.coupon_id)*.10 + (SELECT COUNT(*) FROM clients_coupon WHERE coupons.coupon_id = clients_coupon.coupon_id AND clients_coupon.used = true)*.90)as total_value,\
...
def coupons_with_loyalty(branch_id):
    coupons_with_loyalty = '(SELECT \
                         ...
                         (SELECT \
                            ...
                            FROM coupons WHERE owner_id = %d AND deleted = false AND coupons.available > 0 AND active=true AND coupons.end_date > now() ORDER BY coupons.start_date DESC) ORDER BY type' % (branch_id, branch_id)
